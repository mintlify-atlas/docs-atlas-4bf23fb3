---
title: "DataExtractor — SQL Server OLTP Data Extraction"
...
<ParamField path="db_client" type="DatabaseClient" required>
  An initialised `DatabaseClient` instance. `DataExtractor` stores it as `self.db` and calls `self.db.get_oltp_connection()` for every extraction, ensuring each operation uses a properly pooled connection.
</ParamField>
