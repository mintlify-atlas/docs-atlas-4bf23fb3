...
description: "ETL Dinámico is a modular Python ETL system that extracts from SQL Server OLTP, transforms data with Pandas, and loads incrementally into a Data Warehouse."
---
...
Each pipeline run is driven by a list of column mapping dictionaries. Each mapping declares:

- **`source_column`** — the column name in the source DataFrame.
- **`transform_type`** — one of `none`, `upper`, `lower`, `year`, `month`, `day`, or the special type `concat` (which combines two source columns).
- **`target_column`** — the column name to write in the destination DataFrame.
