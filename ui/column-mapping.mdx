---
title: "Configuring Column Mappings and Transformations"
...
<Accordion title="Business key must be in the mapped target columns">
  The value selected as the business key must appear as `target_column` in at least one regular mapping (not a concat). This guarantees the incremental deduplication key is actually present in the DataFrame being loaded.
</Accordion>
