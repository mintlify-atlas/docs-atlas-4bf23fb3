...
Schema filtering uses a case-insensitive comparison (`schema.lower() in system_schemas`) so mixed-case schema names like `INFORMATION_SCHEMA` are correctly excluded regardless of how SQL Server returns them.
