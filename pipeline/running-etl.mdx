...
<ParamField path="target_table" type="str" required>
  Name of the destination table in the OLAP Data Warehouse (e.g., `"DimCustomers"`). Must already exist in the DW — the loader appends to it and will not create it.
</ParamField>
...
<ParamField path="sql_query" type="str | None">
  An optional custom SQL query string. When provided, this overrides `source_table` extraction — `DataExtractor.extract_by_query()` is called instead of `extract_by_table()`. Defaults to `None`.
</ParamField>
